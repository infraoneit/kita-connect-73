...
import { Search, Filter, Download, Plus, Edit2, Phone, Mail, Users, Baby, FileText, ChevronRight, X, Table2, LayoutList } from 'lucide-react';
...
import { VerwaltungTableView } from '@/components/admin/VerwaltungTableView';
...
export default function VerwaltungPage() {
  ...
  // Check if child is exiting soon (within 30 days)
  const isExitingSoon = (child: any) => {
    if (!child.contracts || child.contracts.length === 0) return false;
    const activeContract = child.contracts.find((c: any) => c.status === 'active');
    if (!activeContract?.end_date) return false;
    const daysUntilExit = differenceInDays(new Date(activeContract.end_date), new Date());
    return daysUntilExit >= 0 && daysUntilExit <= 30;
  };
  ...
  return (
    <div className="min-h-screen pb-24">
      ...
      <div className="p-4 space-y-4">
        {/* Table View */}
        {viewMode === 'table' && activeTab === 'children' && (
          <VerwaltungTableView
        ...
        {/* Card View or other tabs */}
        {(viewMode === 'cards' || activeTab !== 'children') && (
          <>
            {/* Search & Filter Bar */}
            <div className="flex gap-2">
                ...
                {isLoading ? (
                  ...
                  filteredChildren.map(child => (
                    <Card 
                      key={child.id} 
                      className={cn(
                        "border-border cursor-pointer hover:bg-secondary/30 transition-colors",
                        isExitingSoon(child) && "bg-exiting-light border-exiting"
                      )}
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div 
                              ...
                              style={{ backgroundColor: child.groups?.color || 'hsl(var(--primary))' }}
                            >
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                {isExitingSoon(child) && (
                                  <Badge className="bg-exiting text-exiting-foreground h-5 text-[10px]">
                                    Austritt bald
                                  </Badge>
                                )}
                              </div>
